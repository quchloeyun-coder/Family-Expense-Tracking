...
function NumPad(props){
  ...
  return h('div',{style:{display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:4,padding:"0 2px"}},keys.map(function(k){return h('button',{key:k,onClick:function(){tap(k)},style:{height:42,border:"none",borderRadius:10,fontSize:20,fontWeight:500,background:k==="⌫"?(pressed===k?"#fbb":"#ffe0e0"):(pressed===k?"#d5d7e0":"#f5f5f5"),color:k==="⌫"?"#e74c3c":"#333",cursor:"pointer",transition:"background .1s",transform:pressed===k?"scale(0.95)":"scale(1)"}},k)}));
}

function CatPicker(props){
  var selCat=props.selCat,onSelect=props.onSelect;
  var _pr=us(null),pr=_pr[0],setPr=_pr[1];
  return h('div',{style:{display:"grid",gridTemplateColumns:"repeat(4,1fr)",gap:3}},CATS.map(function(c){var s=selCat&&selCat.id===c.id;var p=pr===c.id;return h('button',{key:c.id,onClick:function(){setPr(c.id);setTimeout(function(){setPr(null)},150);onSelect(c)},style:{display:"flex",flexDirection:"column",alignItems:"center",padding:"5px 2px",border:s?"2px solid "+c.color:"2px solid transparent",borderRadius:10,background:s?c.color+"15":p?"#eee":"#fafafa",cursor:"pointer",transition:"all .1s",transform:p?"scale(0.93)":"scale(1)"}},h('span',{style:{fontSize:18}},c.icon),h('span',{style:{fontSize:9,marginTop:1,color:"#555"}},c.name))}));
}
...
function CurrModal(props){
  ...
  return h('div',{style:{position:"fixed",inset:0,background:"rgba(0,0,0,.4)",display:"flex",alignItems:"center",justifyContent:"center",zIndex:1000},onClick:onClose},h('div',{style:{background:"#fff",borderRadius:16,padding:20,width:360,maxWidth:"92vw",maxHeight:"80vh",overflow:"auto"},onClick:function(e){e.stopPropagation()}},h('div',{style:{fontSize:16,fontWeight:700,marginBottom:8,color:"#333"}},"货币管理"),h('div',{style:{fontSize:11,color:"#999",marginBottom:12}},"修改汇率仅影响之后的记账，已有记录不变"),items,h('div',{style:{display:"flex",gap:8,marginTop:16}},h('button',{onClick:function(){var finalList=list;if(ei!==-1){var r=parseFloat(er);if(!isNaN(r)&&r>0){finalList=list.map(function(c2,i2){return i2===ei?Object.assign({},c2,{rate:r}):c2})}}onSave(finalList);onClose()},style:{flex:1,padding:12,border:"none",borderRadius:10,background:"linear-gradient(135deg,#667eea,#764ba2)",color:"#fff",fontSize:14,fontWeight:700,cursor:"pointer"}},"保存"),h('button',{onClick:onClose,style:{flex:1,padding:12,border:"1px solid #ddd",borderRadius:10,background:"#fff",color:"#888",fontSize:14,cursor:"pointer"}},"取消"))));
}
...
function App(){
  ...
  var _editRec=us(null),editRec=_editRec[0],setEditRec=_editRec[1];
  var _srch=us(""),searchQ=_srch[0],setSrch=_srch[1];
  var _showSrch=us(false),showSearch=_showSrch[0],setShowSearch=_showSrch[1];
  ...
  function handleEditSave(updates){
    ...
    window.FamilyStorage.putRecord(updated).then(function(){return reload()}).then(function(){showT("已修改");setEditRec(null);doSync(null,true)});
  }
  ...
  if(tab==="add"){
    ...
    tabContent=h('div',{style:{display:"flex",flexDirection:"column",height:"calc(100vh - 70px)",overflow:"hidden"}},
      h('div',{style:{background:"linear-gradient(135deg,#667eea,#764ba2)",color:"#fff",borderRadius:"0 0 24px 24px",paddingTop:"max(env(safe-area-inset-top,0px),48px)"}},
        h('div',{style:{display:'flex',justifyContent:'space-between',alignItems:'center',fontSize:11,padding:"0 16px 4px"}},
          h('div',{style:{display:'flex',alignItems:'center',gap:6,opacity:.85}},h('span',{style:{width:6,height:6,borderRadius:'50%',background:online?'#7CFC9D':'#ffb74d',display:'inline-block'}}),h('span',null,online?'在线':'离线'),pc>0?h('span',{style:{background:'rgba(255,255,255,.25)',padding:'1px 6px',borderRadius:8}},pc+"待同步"):null),
          h('button',{onClick:function(){doSync()},disabled:syncing,style:{background:'rgba(255,255,255,.2)',border:'none',color:'#fff',borderRadius:12,padding:'4px 12px',fontSize:11,cursor:syncing?'default':'pointer'}},syncing?'⟳ 同步中':'🔄 刷新')),
        h('div',{style:{padding:"4px 20px 14px",textAlign:"center"}},h('div',{style:{fontSize:12,opacity:.8}},selCat?selCat.icon+" "+selCat.name:"请选择分类"),h('div',{style:{fontSize:28,fontWeight:700,margin:"2px 0"}},curr.symbol+" "+amount),selCurrency!=="HKD"&&a>0?h('div',{style:{fontSize:12,opacity:.8}},"≈ HK$ "+fmtN(a*curr.rate)):null,h('div',{style:{fontSize:11,opacity:.6,marginTop:2}},me.avatar+" "+me.name+" 记账中"))),
      ...
      h('div',{style:{background:"#fff",borderRadius:14,margin:"6px 14px 0",padding:"8px 10px",boxShadow:"0 2px 8px rgba(0,0,0,.04)"}},h(CatPicker,{selCat:selCat,onSelect:function(c){setSC(c)}})),
      h('div',{style:{flex:1,minHeight:4}}),
      h('div',{style:{background:"#fff",borderRadius:14,margin:"0 14px 12px",padding:"8px 10px",boxShadow:"0 2px 8px rgba(0,0,0,.04)"}},
    ...
    var filteredRecs=mRecs;
    if(searchQ){var q=searchQ.toLowerCase();filteredRecs=mRecs.filter(function(r){var catMatch=(r.categoryName||"").toLowerCase().includes(q);var noteMatch=(r.note||"").toLowerCase().includes(q);var amtMatch=String(r.amount).includes(q)||String(r.hkdAmount||"").includes(q);return catMatch||noteMatch||amtMatch})}
    var gd=groupByDate(filteredRecs);
    var homeExp=filteredRecs.reduce(function(s,r){return s+(r.hkdAmount||r.amount)},0);
    tabContent=h('div',null,TopBar,
      h('div',{style:{background:"linear-gradient(135deg,#667eea,#764ba2)",color:"#fff",padding:"8px 20px 20px",borderRadius:"0 0 24px 24px"}},
        h('div',{style:{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:12}},h('button',{onClick:function(){setVM(new Date(viewMonth.getFullYear(),viewMonth.getMonth()-1,1))},style:{background:"rgba(255,255,255,.2)",border:"none",color:"#fff",borderRadius:8,padding:"4px 12px",cursor:"pointer"}},"‹"),h('span',{style:{fontSize:17,fontWeight:600}},mStr(viewMonth)),h('div',{style:{display:"flex",gap:8,alignItems:"center"}},h('button',{onClick:function(){setShowSearch(!showSearch);if(showSearch)setSrch("")},style:{background:"rgba(255,255,255,.2)",border:"none",color:"#fff",borderRadius:8,padding:"4px 10px",cursor:"pointer",fontSize:14}},showSearch?"✕":"🔍"),h('button',{onClick:function(){setVM(new Date(viewMonth.getFullYear(),viewMonth.getMonth()+1,1))},style:{background:"rgba(255,255,255,.2)",border:"none",color:"#fff",borderRadius:8,padding:"4px 12px",cursor:"pointer"}},"›"))),
        showSearch?h('div',{style:{marginBottom:8}},h('input',{value:searchQ,onChange:function(e){setSrch(e.target.value)},placeholder:"搜索分类、备注、金额...",style:{width:"100%",padding:"8px 12px",border:"none",borderRadius:10,fontSize:13,outline:"none",boxSizing:"border-box",background:"rgba(255,255,255,.2)",color:"#fff"}})):null,
        h('div',{style:{textAlign:"center"}},h('div',{style:{fontSize:12,opacity:.8}},searchQ?"搜索结果":"本月支出"),h('div',{style:{fontSize:28,fontWeight:700}},"HK$ "+fmtN(homeExp)),h('div',{style:{fontSize:12,opacity:.7,marginTop:4}},filteredRecs.length+"笔"))),
      filteredRecs.length===0?h('div',{style:{textAlign:"center",padding:60,color:"#bbb"}},h('div',{style:{fontSize:48,marginBottom:12}},"📒"),searchQ?"未找到匹配记录":"本月暂无记录"):
        ...
        return h('div',{key:date,style:CS.card},
          ...
          recs.map(function(r){var mem=fMem(r.member),cur=fCur(r.currency||"HKD",currencies),ih=(r.currency||"HKD")==="HKD";
            return h('div',{key:r.id,onClick:function(){setEditRec(r)},style:{display:"flex",alignItems:"center",padding:"8px 0",borderBottom:"1px solid #f5f5f5",cursor:"pointer"}},
              ...
              h('div',{style:{flex:1,marginLeft:12}},h('div',{style:{fontSize:14,fontWeight:500,color:"#333"}},r.categoryName,r._pending?h('span',{style:{marginLeft:6,fontSize:9,color:'#ffa726',background:'#fff3e0',padding:'1px 5px',borderRadius:6}},"待同步"):null),h('div',{style:{fontSize:11,color:"#aaa",marginTop:2}},mem.avatar+" "+mem.name+(r.note?" · "+r.note:"")+(!ih&&r.amount>0?" · 汇率"+(r.hkdAmount/r.amount).toFixed(2):""))),
