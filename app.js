...
function CurrModal(props){
  ...
  return h('div',{style:{position:"fixed",inset:0,background:"rgba(0,0,0,.4)",display:"flex",alignItems:"center",justifyContent:"center",zIndex:1000},onClick:onClose},h('div',{style:{background:"#fff",borderRadius:16,padding:20,width:360,maxWidth:"92vw",maxHeight:"80vh",overflow:"auto"},onClick:function(e){e.stopPropagation()}},h('div',{style:{fontSize:16,fontWeight:700,marginBottom:8,color:"#333"}},"货币管理"),h('div',{style:{fontSize:11,color:"#999",marginBottom:12}},"修改汇率仅影响之后的记账，已有记录不变"),items,h('div',{style:{display:"flex",gap:8,marginTop:16}},h('button',{onClick:function(){onSave(list);onClose()},style:{flex:1,padding:12,border:"none",borderRadius:10,background:"linear-gradient(135deg,#667eea,#764ba2)",color:"#fff",fontSize:14,fontWeight:700,cursor:"pointer"}},"保存"),h('button',{onClick:onClose,style:{flex:1,padding:12,border:"1px solid #ddd",borderRadius:10,background:"#fff",color:"#888",fontSize:14,cursor:"pointer"}},"取消"))));
}
...
function App(){
  ...
  if(tab==="add"){
    ...
    tabContent=h('div',{style:{display:"flex",flexDirection:"column",height:"calc(100vh - 70px)",overflow:"hidden"}},TopBar,
      ...
      h('div',{style:{flex:1}}),
      h('div',{style:{background:"#fff",borderRadius:14,margin:"0 14px 4px",padding:"6px 10px",boxShadow:"0 2px 8px rgba(0,0,0,.04)"}},
        ...
        h('button',{onClick:save,style:{width:"100%",padding:9,marginTop:4,border:"none",borderRadius:12,fontSize:15,fontWeight:700,background:"linear-gradient(135deg,#667eea,#764ba2)",color:"#fff",cursor:"pointer"}},"记一笔 ✓")));
  }else if(tab==="home"){
